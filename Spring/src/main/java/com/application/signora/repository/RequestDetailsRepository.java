...
public interface RequestDetailsRepository extends JpaRepository<RequestDetails, Long> {
    ...
    @Query(value = """
            ...
            "APPROVED_BY_FACULTY", "REJECTED_BY_FACULTY", "MOVED_TO_FACULTY")
            """, nativeQuery = true)
    ...
    @Query(value = """
            ...
            JOIN REQUEST_DETAILS RD ON RD.STUDENT_ID = STUD.ID
            WHERE AUTH.REGISTERED_USER_ID = :IDENTIFIER
            AND RD.STATUS NOT IN ("APPROVED_BY_FACULTY", "REJECTED_BY_FACULTY",
            "APPROVED_BY_HOD", "REJECTED_BY_HOD", "MOVED_TO_FACULTY", "MOVED_TO_HOD")
            """, nativeQuery = true)
